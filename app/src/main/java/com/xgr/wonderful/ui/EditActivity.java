...
//			startActivityForResult(camera, REQUEST_CODE_CAMERA);
	...
	// 拍照获取照片
	private ActivityResultLauncher<Void> requestCameraFile = registerForActivityResult(new ActivityResultContracts.TakePicturePreview(), bitmap -> {
	...
	// 相册获取照片
	ActivityResultLauncher<String> requestPhotoFile = registerForActivityResult(new ActivityResultContracts.GetContent(), result -> {
		try {
	...
	// private ActivityResultLauncher<Uri> requestPhotoFile =
//	registerForActivityResult(ActivityResultContracts.GetContent(),)
	...
	private void publish(final String commitContent) {
		...
		figureFile.upload(new UploadFileListener() {
			...
			public void done(BmobException e) {
				if (e != null) {
					LogUtils.i(TAG, "上传文件失败。" + e.getMessage());
				} else {
	...
	private void publishWithoutFigure(final String commitContent,
		...
		qiangYu.save(new SaveListener<String>() {
			@Override
			public void done(String s, BmobException e) {
				if (e != null) {
					ActivityUtil.show(EditActivity.this, "发表失败！yg" + e.getMessage());
					LogUtils.i(TAG, "创建失败。" + e.getMessage());
				} else {
