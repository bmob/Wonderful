...
public class PullToRefreshWebView extends PullToRefreshBase<WebView> {
	...
	protected boolean isReadyForPullEnd() {
		float density = mRefreshableView.getResources().getDisplayMetrics().density;
		double exactContentHeight = Math.floor(mRefreshableView.getContentHeight() * density);
	...
	final class InternalWebViewSDK9 extends WebView {
		...
		private int getScrollRange() {
			float density = mRefreshableView.getResources().getDisplayMetrics().density;
			return (int) Math.max(0, Math.floor(mRefreshableView.getContentHeight() * density)
